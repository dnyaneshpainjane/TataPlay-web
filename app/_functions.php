...
function fetchepid($id)
{
    global $TPAUTH;
    $mepiID = '';
    $selectID = array();
    $getsubsch = @json_decode(@file_get_contents('secure/channels'), true);
    $entids = $TPAUTH;
    
    if(!empty($getsubsch))
    {
        foreach($getsubsch as $baddchn)
        {
            if($id == $baddchn['id'])
            {
                $selectID = $baddchn['entitlements'];
            }
        }
    }
    ...
    if(!empty($selectID))
    {
        foreach($selectID as $npchn)
        {
            foreach($entids['entitlements'] as $userent)
            {
                if($userent['pkgId'] == $npchn)
                {
                    $mepiID = $npchn;
                }
            }
