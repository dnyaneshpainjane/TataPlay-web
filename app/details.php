...
if(file_exists('secure/streams/'.$id))
{
    ...
    if(stripos($iJChData, '///') !== false)
    {
        ...
        if(time() < $iscac_time)
        {
            header("X-TP-Cache: HIT");
            api_res('success', 200, 'OK', @json_decode($iscac_data, true));
        }
    }
}
...
if(isset($vUData['message']) && stripos($vUData['message'], 'success') !== false)
{
    ...
    if(!empty($chnSerialID) && !empty($playurl) && !empty($licurl))
    {
        ...
        header("X-TP-Cache: MISS");
        api_res('success', 200, 'OK', $tv_details);
    }
    
}

api_res('error', 404, 'Channel Not Found', '');
